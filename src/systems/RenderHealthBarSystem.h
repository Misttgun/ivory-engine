	...
	public:
		...
		void Draw(SDL_Renderer* renderer, const std::unique_ptr<ResourceManager>& assetStore, const SDL_Rect& camera) const
		{
			for (const auto entity : GetEntities())
			{
				...
				const int32 healthPercentage = static_cast<int>((static_cast<float>(health.m_currentHealth) / static_cast<float>(health.m_maxHealth)) * 100);
				...
				const int32 healthBarPosX = static_cast<int>(transform.m_position.x + static_cast<float>(sprite.m_width) * transform.m_scale.x) - camera.x;
				const int32 healthBarPosY = static_cast<int>(transform.m_position.y) - camera.y;
