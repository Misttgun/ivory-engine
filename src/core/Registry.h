...
namespace re
{
	...
	void Registry::AddComponent(const Entity entity, T component)
	{
		...
		if (m_componentPools.contains(typeIndex) == false)
	...
	void Registry::RemoveSystem()
	{
		...
		SDL_assert(m_systems.contains(typeIndex) && "Trying to remove system before registered.");
	...
	T& Registry::GetSystem() const
	{
		...
		SDL_assert(m_systems.contains(typeIndex) && "System used before registered.");
